...
function renderContributionsCard(username, contributionDays, theme) {
  ...
  <path d="${pathD}" fill="none" stroke="${colors.iconColor}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>

  ${points.map((p) => `<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="3" fill="${colors.iconColor}" stroke="${colors.bgColor}" stroke-width="1" opacity="0.95"/>`).join('\n  ')}
